    ...
    try {
        ...
        if (Array.isArray(clients)) {
            clients.forEach(client => {
                const row = `
                    <tr>
                        <td style="font-weight: 600;">${client.nama}</td>
                        <td>${client.sektor}</td>
                        <td>${client.alamat}</td>
                        <td>
                            <div class="action-btns">
                                <button class="btn-icon btn-edit" onclick="bukaModal('edit', ${client.id})">
                                    <i class="fas fa-edit"></i>
                                </button>
                                <button class="btn-icon btn-delete" onclick="hapusKlien(${client.id})">
        ...
        if (orgData.length === 0) {
            container.innerHTML = `
                <div class="empty-state" style="text-align: center; padding: 50px; background: #f9f9f9; border-radius: 12px; border: 2px dashed #ddd;">
                    ...
                    <p style="color: #888;">Belum ada struktur organisasi.</p>
                    <button class="btn-add" onclick="bukaModalOrg('divisi', 'tambah')" style="margin: 15px auto;">
            ...
            let divisiHtml = `
                <div class="org-level">
                    <div class="level-header">
                        ...
                        <div class="action-btns">
                            <button class="btn-icon btn-edit" title="Edit Divisi" onclick="bukaModalOrg('divisi', 'edit', ${divisi.id})"><i class="fas fa-edit"></i></button>
                            <button class="btn-icon btn-delete" title="Hapus Divisi" onclick="hapusOrg('divisi', ${divisi.id})"><i class="fas fa-trash"></i></button>
                        </div>
                    </div>
                    <div class="nested-container">
                        ...
                        ${(divisi.departments || []).map(dept => `
                            <div class="org-level department-item">
                                <div class="level-header">
                                    <div class="level-title">
                                        <button class="btn-toggle">-</button>
                                    ...
                                    <div class="action-btns">
                                        <button class="btn-icon btn-edit" title="Edit Department" onclick="bukaModalOrg('department', 'edit', ${dept.id}, ${divisi.id})"><i class="fas fa-edit"></i></button>
                                        <button class="btn-icon btn-delete" title="Hapus Department" onclick="hapusOrg('department', ${dept.id}, ${divisi.id})"><i class="fas fa-trash"></i></button>
                                    </div>
                                </div>
                                <div class="nested-container">
                                    ...
                                    ${(dept.positions || []).map(pos => `
                                        <div class="org-level position-item">
                                            <div class="level-header">
                                                <div class="level-title">
                                                    <div style="display: flex; align-items: center; gap: 15px;">
                                                        <div style="width: 35px; height: 35px; background: #eee; border-radius: 50%; display: grid; place-items: center;">
                                                            <i class="fas fa-user" style="color: #95a5a6;"></i>
                                                        </div>
                                                        <div style="display: flex; flex-direction: column;">
                                                            <span style="font-weight: 700; color: #2c3e50;">${pos.employee_name || '-'}</span>
                                                            <span style="font-weight: 500; font-size: 13px; color: var(--primary-dark);">${pos.nama}</span>
                                                            <span style="font-size: 11px; color: var(--text-muted);"><i class="fas fa-envelope"></i> ${pos.email || '-'} | <i class="fas fa-phone"></i> ${pos.phone || '-'}</span>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="action-btns">
                                                    <button class="btn-icon btn-edit" title="Edit Posisi" onclick="bukaModalOrg('posisi', 'edit', ${pos.id}, ${dept.id})"><i class="fas fa-edit"></i></button>
                                                    <button class="btn-icon btn-delete" title="Hapus Posisi" onclick="hapusOrg('posisi', ${pos.id}, ${dept.id})"><i class="fas fa-trash"></i></button>
                                                </div>
                                            </div>
                                        </div>
                                    `).join('')}
    ...
    formOrg.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            if (id && id !== "" && id !== "null") {
                ...
                let endpoint = '';
                let body = { nama: name };
                if (type === 'divisi') endpoint = '/divisions';
                else if (type === 'department') {
